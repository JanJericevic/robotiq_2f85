...
class GripperJointPublisher:
    def __init__(self, prefix):
        self.prefix = prefix
        self.pub = rospy.Publisher('gripper_joint_states', JointState, queue_size=10)
    ...
    def callback(self, data):
        ...
        state.name = [self.prefix + 'finger_joint']
        ...
        # state.position = [(1 - (float(data.gPO) / 255.0)) * 0.755]
        state.velocity = [0.0]
        state.effort = [0.0]
...
if __name__ == '__main__':
    rospy.init_node('robotiq_2f_85_joint_state_publisher')
